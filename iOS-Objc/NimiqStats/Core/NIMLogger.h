...
#ifndef NIMDebugLogResponse
#define NIMDebugLogResponse(response, responseObject,request, error) [[NIMLogger sharedLogger] logResponse:response responseObject:responseObject request:request error:error];
#endif
 ...
 */
#ifndef NIMDebugLog
#define NIMDebugLog(format, ...) [[NIMLogger sharedLogger] debugLog:[NSString stringWithFormat:@"%s", __PRETTY_FUNCTION__] message: format, ##__VA_ARGS__];
#endif
...
/**
 * Enabled switch, default NO - default set by NIMConfiguration debug property
 */
