#import "NIMSanitizer.h"
#import "NIMObject.h"
#import "NIMQueryParamCollection.h"
#import "NIMDefaultConfiguration.h"
#import <ISO8601/ISO8601.h>

NSString * const kNIMApplicationJSONType = @"application/json";

NSString * NIMPercentEscapedStringFromString(NSString *string) {
    static NSString * const kNIMCharactersGeneralDelimitersToEncode = @":#[]@";
    static NSString * const kNIMCharactersSubDelimitersToEncode = @"!$&'()*+,;=";

    NSMutableCharacterSet * allowedCharacterSet = [[NSCharacterSet URLQueryAllowedCharacterSet] mutableCopy];
    [allowedCharacterSet removeCharactersInString:[kNIMCharactersGeneralDelimitersToEncode stringByAppendingString:kNIMCharactersSubDelimitersToEncode]];
...
- (id) sanitizeForSerialization:(id) object {
    ...
    else if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]] || [object isKindOfClass:[NIMQueryParamCollection class]]) {
        return object;
    }
    else if ([object isKindOfClass:[NSDate class]]) {
        return [NIMSanitizer dateToString:object];
    }
    ...
    else if ([object isKindOfClass:[NIMObject class]]) {
...
- (NSString *) parameterToString:(id)param {
    if ([param isKindOfClass:[NSString class]]) {
        ...
        return [NIMSanitizer dateToString:param];
    }
...
+ (NSString *)dateToString:(id)date {
    NSTimeZone* timeZone = [NIMDefaultConfiguration sharedConfig].serializationTimeZone;
...
- (NSString *) selectHeaderAccept:(NSArray *)accepts {
    ...
    for (NSString *string in accepts) {
        if ([self.jsonHeaderTypeExpression matchesInString:string options:0 range:NSMakeRange(0, [string length])].count > 0) {
            return kNIMApplicationJSONType;
        }
